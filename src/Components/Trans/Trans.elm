...
import Lib.Component.Base exposing (dsetstring)
import Lib.Component.Base exposing (dsetbool)
import Lib.Component.Base exposing (dgetint)
import Lib.Component.Base exposing (dsetint)
import Lib.Component.Base exposing (dgetbool)
import Lib.Component.Base exposing (dgetString)
import Lib.Render.Render exposing (renderSprite)

pausetime : Int     --the length of the movement time
pausetime = 
    100
...
updateTrans msg gMsg globalData ( d, t ) =
    ...
    case gMsg of
        ...
        _ ->
            if localtime ==  pausetime && dgetString d "mode" == "start"
            then
                ( d, ComponentStringMsg "transEnd", globalData )

            else if localtime ==  pausetime && dgetString d "mode" == "end"
            then
